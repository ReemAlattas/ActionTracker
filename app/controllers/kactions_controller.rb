class KactionsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @kaction.update(kaction_params)
        format.html { redirect_to @kaction, notice: 'Action was successfully updated.' }
  ...
  def destroy
    @kaction.destroy
    respond_to do |format|
      format.html { redirect_to kactions_url, notice: 'Action was successfully destroyed.' }
  ...
  private
    ...
    def kaction_params
      params.require(:kaction).permit(:description, :target)
    end
end
